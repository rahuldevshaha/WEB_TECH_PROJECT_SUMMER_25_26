    ...
    <link rel="stylesheet" href="/app/assets/css/addCost.css">
    ...
    <div class="addCost_page main-container">
        ...
        <form method="POST" action="">
            ...
            <?php if (!isset($costTypeOptions) || empty($costTypeOptions)): ?>
                <?php $costTypeOptions = array("Meal Cost", "Gas Bill", "Electricity Bill", "WiFi Bill", "Other"); ?>
            <?php endif; ?>
            ...
            <div class="form-group">
                ...
                <select name="costType" class="form-control" required>
                    ...
                    <?php foreach ($costTypeOptions as $type): ?>
                        <option value="<?php echo $type; ?>" <?php if(isset($costType) && $costType == $type) echo 'selected'; ?>>
                            <?php echo $type; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                <label>Date</label>
                <input type="date" name="costDate" class="form-control" value="<?php echo isset($costDate) ? $costDate : date('Y-m-d'); ?>" required>
            </div>

            <div class="form-group">
                <label>Amount</label>
                <input type="number" step="0.01" name="amount" class="form-control" placeholder="e.g. 1000" value="<?php echo isset($amount) ? $amount : ''; ?>" required>
            </div>

            <div class="form-group">
                <label>Note (optional)</label>
                <textarea name="note" class="form-control" placeholder="e.g. rice, oil, fish etc."><?php echo isset($note) ? $note : ''; ?></textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="costBy" class="form-control" required>
                    ...
                    <?php $members = isset($members) ? $members : array(); ?>
                    <?php foreach ($members as $m): ?>
                        <option value="<?php echo $m['userId']; ?>" <?php if((isset($costBy) ? $costBy : $currentUserId) == $m['userId']) echo 'selected'; ?>>
                            <?php echo htmlspecialchars($m['Name']); ?><?php if(isset($m['Role']) && $m['Role'] == 'Manager') echo ' (Manager)'; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="checkbox-row">
                <input type="checkbox" id="fundCheck" name="auto_fund" <?php if(!isset($_POST["add_cost_submit"]) || isset($_POST["auto_fund"])) echo 'checked'; ?>>
                <label for="fundCheck">সমপরিমাণ টাকা তার নামে জমা করুন?</label>
            </div>

            <button type="submit" name="add_cost_submit" class="primary-btn">Add Cost</button>
        </form>
    ...
    <div id="costSuccessModal" class="modal-overlay <?php if($showModal) echo 'active'; ?>">
        <div class="modal-card">
            ...
            <h3 class="modal-title"><span>🍅</span> খরচ যুক্ত হয়েছে</h3>
            <p class="modal-subtitle">হিসাব চেক করুন মাসের বিস্তারিত হিসাব থেকে</p>

            <div class="modal-btn-group">
                <a href="monthlyDetails.php" class="modal-btn-light">মাসের বিস্তারিত হিসাব</a>
                <a href="addCost.php" class="modal-btn-light">আরো খরচ যুক্ত করুন</a>
                <a href="dashboard.php" class="modal-btn-red">Go to Dashboard</a>
            </div>
        </div>
    </div>
