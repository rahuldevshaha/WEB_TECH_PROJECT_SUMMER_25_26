...
function runMemberSearch(term) {
  ...
  memberSearchXhrController.open(
    "GET",
    "/app/controller/components/messMember.php?search_members=" +
  ...
  memberSearchXhrController.onreadystatechange = function () {
    if (memberSearchXhrController.readyState === 4) {
      if (
        memberSearchXhrController.status >= 200 &&
        memberSearchXhrController.status < 300
      ) {
        try {
...
function buildMemberCardElement(m, isManager) {
  ...
  emailRow.innerHTML =
    '<span class="email-left"><strong>Email:</strong> <span id="' +
    emailId +
    '"></span></span>' +
...
var calendarLockedPastDates = new Set();
...
function renderCalendarGrid() {
  ...
  for (var d = 1; d <= daysInMonth; d++) {
    (function (d) {
      ...
      var isLockedPast = calendarLockedPastDates.has(dateStr);
      ...
      if (isLockedPast) {
        ...
        if (isPast) {
          cell.title = isMine
            ? "Selected past date"
            : "Past date (can still be assigned)";
        }
...
function openCalendarModal(userId, existingDates) {
  ...
  var initialDates = Array.isArray(existingDates) ? existingDates : [];

  calendarSelectedDates = new Set(initialDates);
  ...
  calendarLockedPastDates = new Set(
    initialDates.filter(function (d) {
      return d < today;
    }),
  );
...
function prepareBazarSubmit() {
  ...
  if (datesInput) {
    datesInput.value = JSON.stringify(Array.from(calendarSelectedDates));
  }
...
function savePermissionAndShowPopup() {
  ...
  if (permSelect && permSelect.value) {
    ...
    var successModal = document.getElementById("permSuccessModal");
