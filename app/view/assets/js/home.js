(function () {
  ...
  function scopeCssText(cssText) {
    ...
    while (i < len) {
      ...
      if (prelude.charAt(0) === "@") {
        var atName = (prelude.match(/^@([a-zA-Z-]+)/) || [
          "",
          "",
        ])[1].toLowerCase();
  ...
  function ensureScopedCss(href) {
    ...
    var promise = fetch(href, { credentials: "same-origin" })
      ...
      .then(function (cssText) {
        return sanitizeComponentCss(cssText);
      })
      .catch(function (err) {
  ...
  function extractStylesheets(container, token) {
    ...
    document
      .querySelectorAll("style[data-component-style]")
      .forEach(function (el) {
        el.remove();
      });

    hrefs.forEach(function (href) {
      ensureScopedCss(href)
        .then(function (scopedCss) {
      ...
      form.addEventListener("submit", function (e) {
        ...
        fetch(actionUrl, {
          ...
          .catch(function (err) {
            ...
            console.error(err);
            renderInto(
              '<p class="error_text">কন্টেন্ট লোড করতে সমস্যা হয়েছে। আবার চেষ্টা করুন।</p>',
              token,
            );
          });
      });
  ...
  document.addEventListener("click", function (e) {
    ...
    if (!url) {
      ...
      renderInto(
        '<p class="empty_state">এই ফিচারটি শীঘ্রই আসছে।</p>',
        skipToken,
      );
      return;
    }
