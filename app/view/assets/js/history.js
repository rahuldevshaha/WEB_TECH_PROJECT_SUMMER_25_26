document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderHistory() {
    ...
    cards.forEach(function (card) {
      ...
      const matched = query === "" || month.includes(query);
    ...
    visibleCards.sort(function (a, b) {
      const aIndex = Number(a.getAttribute("data-index")) || 0;

      const bIndex = Number(b.getAttribute("data-index")) || 0;

      const aExpense = Number(a.getAttribute("data-expense")) || 0;

      const bExpense = Number(b.getAttribute("data-expense")) || 0;

      const aMeal = Number(a.getAttribute("data-meal")) || 0;

      const bMeal = Number(b.getAttribute("data-meal")) || 0;
  ...
  window.openHistoryModal = function (button) {
    ...
    document.getElementById("modalMeal").textContent = data.dataset.meal || "0";

    document.getElementById("modalRate").textContent =
      (data.dataset.currency || "BDT") + " " + (data.dataset.rate || "0");

    document.getElementById("modalExpense").textContent =
      (data.dataset.currency || "BDT") + " " + (data.dataset.expense || "0");

    document.getElementById("modalFund").textContent =
      (data.dataset.currency || "BDT") + " " + (data.dataset.fund || "0");

    document.getElementById("modalDue").textContent =
      (data.dataset.currency || "BDT") + " " + (data.dataset.due || "0");
